...
class UserCreateProfileView(APIView):
    ...
    def post(self, request, format=None):
        serializer = UserCreateProfileSerializer(data=request.data, context={'user': request.user})
        if serializer.is_valid(raise_exception=True):
            serializer.save()
            return Response({'msg': 'Profile Successfully Created'}, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    
    def get(self, request, format=None):
        try:
            profile = Profile.objects.get(user=request.user.id)
            return Response({'msg': 'have a profile'}, status=status.HTTP_200_OK) if profile is not None else Response({'msg': 'no profile'}, status=status.HTTP_404_NOT_FOUND)
