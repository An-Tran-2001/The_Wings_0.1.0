...
class UserLoginView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            user = User.objects.filter(email=username_email).first() if '@' in username_email else User.objects.filter(username=username_email).first()
            if user is None:
                return Response({'detail': 'User is not valid'}, status=status.HTTP_400_BAD_REQUEST)
            else:
