...
class PostsViewSet(RetrieveModelMixin, ListModelMixin, GenericViewSet):
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        if friend:= Friend.objects.filter(Q(Q(user=self.request.user) | Q(friend=self.request.user) & Q(is_accepted=True))).values('user', 'friend'):
            return self.queryset.filter(Q(author=self.request.user.id) | Q(author__in=friend)).order_by('-created_at')
        
        return self.queryset.filter(status='public').order_by('-created_at')
    
    def retrieve(self, request, *args, **kwargs):
        post_id = kwargs.get('id', None)
        status_post = self.queryset.filter(id=post_id).values('status').first()["status"]
        if status_post == "private":
            if friend:= Friend.objects.filter(Q(Q(user=self.request.user) | Q(friend=self.request.user) & Q(is_accepted=True))).values('user', 'friend'):
                if self.queryset.filter(Q(author=self.request.user.id) | Q(author__in=friend)).filter(id=post_id).exists():
                    return super().retrieve(request, *args, **kwargs)
            else:
                return Response(status=status.HTTP_403_FORBIDDEN, data = {"message": "You guys are not friends"})
        elif status_post == "private_only":
            return Response(status=status.HTTP_403_FORBIDDEN, data = {"message": "Only author can see this post"})
    ...
    def your_post(self, request, *args, **kwargs):
        ...
        friend = Friend.objects.filter(Q(Q(user=user_id) | Q(friend=user_id) & Q(is_accepted=True))).values('user', 'friend')
        if self.request.user.id in friend:
            serializer = self.serializer_class(
                self.get_queryset().filter(Q(author=user_id) | Q(status="private")), many=True, context={"request": request}
            )
        else:
            serializer = self.serializer_class(
                self.get_queryset().filter(Q(author=user_id) | Q(status="public")), many=True, context={"request": request}
            )
        return Response(status=status.HTTP_200_OK, data={"posts": serializer.data})
